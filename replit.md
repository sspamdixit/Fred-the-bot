...
- Discord moderation includes a non-AI slur filter at the start of `messageCreate`. Matching configured slurs immediately attempts to delete the message, sends a firm DM warning regardless of role, applies a 10-minute timeout when Discord permissions/role hierarchy allow it, logs deletion/warning/timeout failures, and returns before live-feed emission or AI processing so no model tokens are spent.
