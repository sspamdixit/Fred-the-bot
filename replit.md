...
- Fred FM (`/radio`) plays local files through `@discordjs/voice` + `prism-media`, which spawns `ffmpeg` to transcode `.mp3`/`.wav`/`.ogg` into PCM. Hosts without a system `ffmpeg` (e.g. Render's bare Node runtime) used to fail silently — bot would join VC, never emit opus frames, and Discord would drop the connection. The `ffmpeg-static` npm package now ships a static binary as a dependency, and `server/radio.ts` sets `process.env.FFMPEG_PATH` from it on import as a belt-and-braces fallback to prism-media's auto-discovery. The first `/radio` invocation per process also logs the `@discordjs/voice` dependency report so missing ffmpeg/opus/sodium is visible in the radio logs.
