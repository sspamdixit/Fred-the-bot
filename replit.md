...
- Discord moderation includes a non-AI slur filter at the start of `messageCreate`. Matching configured slurs immediately sends a firm DM warning, applies a 10-minute timeout when a guild member is available, logs the action, and returns before live-feed emission or AI processing so no model tokens are spent.
