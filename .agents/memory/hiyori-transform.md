---
name: Hiyori bot transformation
description: Full rebrand of Fred Discord bot to Hiyori waifu AI with gacha/economy
---
...
Rewrote: server/ai-settings.ts (Hiyori persona), server/bot.ts, server/routes.ts, server/storage.ts
DB tables removed: savedPlaylists, playlistTracks — storage.ts stripped of all playlist methods
Trigger word changed from "fred" to "hiyori" in bot.ts message handler

**Why:** User wanted waifu/gacha bot with gem economy for monetization
**How to apply:** Hiyori persona lives in server/ai-settings.ts DEFAULT_SYSTEM_INSTRUCTIONS
